...
var html5Api = {
    ...
    getWNCertInfo: function (callback) {
        var wnCertInfo = KICA_DIALOG.getSelectedCertJSON();
    ...
    getVIDRandom: function (options, callback) {
        var certificate = KICA_DIALOG.getSelectedCert();
        var qlalfqjsgh = KICA_DIALOG.getQaqg();
    ...
    symmetricDecrypt: function (alg, mode, cipherMessage, options) {
        ...
        if (options.key) {
            var iv = undefined;
            if (options.iv) iv = webnpki.bin.decodeHex(options.iv);
...
var html5DialogApi = {
    getWNCertInfo: function (callback) {
        $("#kc-confirm-cert-selection").unbind("click");
        $("#kc-confirm-cert-selection").on("click", function () {
